...
export async function fetchAdminRestaurants(): Promise<RestaurantListItem[]> {
  const { data } = await apiClient.get<RestaurantListItem[]>(API_ENDPOINTS.adminRestaurants);
  return data;
}
...
export async function createRestaurant(
  ...
  const { data } = await apiClient.post<RestaurantListItem>(API_ENDPOINTS.adminRestaurants, payload);
  return data;
}
...
export async function updateRestaurant(
  ...
  const { data } = await apiClient.put<RestaurantListItem>(API_ENDPOINTS.adminRestaurantById(id), payload);
  return data;
}
...
export async function fetchAnalysisJobs(): Promise<AnalysisJob[]> {
  const { data } = await apiClient.get<AnalysisJob[]>(API_ENDPOINTS.adminAnalysisJobs);
  return data;
}

export async function startAnalysisJob(restaurantId: number): Promise<AnalysisJob> {
  const { data } = await apiClient.post<AnalysisJob>(API_ENDPOINTS.adminAnalysisJobs, { restaurantId });
  return data;
}

export async function fetchAnalysisJobById(jobId: string): Promise<AnalysisJob> {
  const { data } = await apiClient.get<AnalysisJob>(API_ENDPOINTS.adminAnalysisJobById(jobId));
  return data;
}
